...
/**
 * Temperature Northbound REST API, Handles adding/removing subscriptions
 *
 * <br>
 * <br>
...
public class TemperatureNorthboundJAXRS {
	 ...
	 *
	 * @param containerName
	 *            The container for which we want to retrieve the String (Eg.
	 *            'default')
	 *
	 * @return A String
	 *
	...
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@StatusCodes({ @ResponseCode(code = 201, condition = "User String added successfully"),
	...
	public Response addUserLink(@PathParam(value = "containerName") String containerName,
		...
		if (stat) {
			NorthboundUtils.auditlog("User String", username, "added",
					userStringConfig.getName() + " with parameter " + userStringConfig.getPortNum() + ", "
							+ userStringConfig.getOperator() + ":" + userStringConfig.getReferenceValue(),
					containerName);
